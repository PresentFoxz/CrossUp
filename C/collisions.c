...
VectMf cylinderInTriangle(float cx, float cy, float cz, float radius, float height) {
    ...
    for (int i = 0; i < collisionCount; i++) {
        ...
        float minX = fminf(tri.x1, fminf(tri.x2, tri.x3));
        float maxX = fmaxf(tri.x1, fmaxf(tri.x2, tri.x3));
        float minZ = fminf(tri.z1, fminf(tri.z2, tri.z3));
        float maxZ = fmaxf(tri.z1, fmaxf(tri.z2, tri.z3));

        if (cx + radius < minX || cx - radius > maxX || cz + radius < minZ || cz - radius > maxZ) { continue; }

        float bottomDist = (cx - tri.x1)*tri.normalX + (cy - tri.y1)*tri.normalY + (cz - tri.z1)*tri.normalZ;
        float topDist    = (cx - tri.x1)*tri.normalX + (cy + height - tri.y1)*tri.normalY + (cz - tri.z1)*tri.normalZ;
        ...
        } else if (fabsf(bottomDist) < radius) {
            t = 0.0f;
        } else {
            t = 1.0f;
        }
        ...
        if (u >= 0 && v >= 0 && (u + v <= 1)) {
            ...
            if (tri.type == OUT_OF_BOUNDS) return (VectMf){0.0f, 0.0f, 0.0f, -1, -1, -1};
