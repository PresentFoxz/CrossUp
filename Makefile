...
directories:
	@mkdir -p $(SIM_BUILD_DIR) $(DEVICE_BUILD_DIR) $(BUILD_DIR)
...
SRCDIR = C
SRC_FOLDERS = playdate Foxgine allFiles
SRCS = $(foreach dir,$(SRC_FOLDERS), \
	$(shell find $(SRCDIR)/$(dir) -name "*.c"))
SIM_OBJS    = $(patsubst %.c,$(SIM_BUILD_DIR)/%.o,$(SRCS))
DEVICE_OBJS = $(patsubst %.c,$(DEVICE_BUILD_DIR)/%.o,$(SRCS))
...
CUSFOLDER = CrossUp-sim
CUDFOLDER = CrossUp-dev
PDC = $(PLAYDATE_SDK_PATH)/bin/pdc.exe
...
pdx-sim: sim_build/pdex.dll
	...
	cp C/pdxinfo $(CUSFOLDER)/
	cp -r $(OBJ_SRC) $(CUSFOLDER)/
	if [ -d "$(AUDIO_SRC)/music" ]; then mkdir -p $(CUSFOLDER)/music && cp -r $(AUDIO_SRC)/music/. $(CUSFOLDER)/music/; fi
	if [ -d "$(AUDIO_SRC)/sfx" ]; then mkdir -p $(CUSFOLDER)/sfx && cp -r $(AUDIO_SRC)/sfx/. $(CUSFOLDER)/sfx/; fi
	cp sim_build/pdex.dll $(CUSFOLDER)/
	$(PDC) $(CUSFOLDER) CrossUp-sim.pdx
...
pdx-dev: device_build/pdex.elf
	...
	cp C/pdxinfo $(CUDFOLDER)/
	cp -r $(OBJ_SRC) $(CUDFOLDER)/
	if [ -d "$(AUDIO_SRC)/music" ]; then mkdir -p $(CUDFOLDER)/music && cp -r $(AUDIO_SRC)/music/. $(CUDFOLDER)/music/; fi
	if [ -d "$(AUDIO_SRC)/sfx" ]; then mkdir -p $(CUDFOLDER)/sfx && cp -r $(AUDIO_SRC)/sfx/. $(CUDFOLDER)/sfx/; fi
	cp device_build/pdex.elf $(CUDFOLDER)/
	cp device_build/pdex_unstripped.elf save_elf
	cp device_build/pdex.elf save_elf

	$(PDC) $(CUDFOLDER) CrossUp-dev.pdx
	zip -r CrossUp.pdx.zip CrossUp-dev.pdx
